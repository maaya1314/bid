...
class GScheduler(object):
    ...
    def _check_sch_config(mod):
        ...
        if hasattr(mod, "cron") and hasattr(mod, "site") and hasattr(mod, "cmd"):
            if not isinstance(mod.cmd, (list, tuple)) or not mod.cmd:
                return False
            if not isinstance(mod.cron, str) or not mod.cron:
                return False
            if not isinstance(mod.site, str) or not mod.site:
                return False
        else:
            return False
        return True
    ...
    def modify_task(self, task_path):
        ...
        try:
            ...
            logger.error("What happen when modify:{}\t{}".format(task_path, e))
            logger.exception(e)
    ...
    def remove_task(self, task_path, kill=True):
        ...
        if kill and task.has_key("process"):
            try:
                ...
                logger.error("What happen when reomve:{}\t[}".format(task_path, e))
                logger.exception(e)
        try:
    ...
    def scan_task_config(self):
        while self.scan_running:
            now_time = time.time()
            # 遍历任务目录
            parent_dirs = os.listdir(self.__tasks_dir)
            for parent_line in parent_dirs:
                parent_task_path = os.path.join(self.__tasks_dir, parent_line)
                # 非文件夹跳过
                if not os.path.isdir(parent_task_path):
                    continue
                parent_task_dirs = os.listdir(parent_task_path)
                for line in parent_task_dirs:
                    ...
                    if os.path.isfile(conf_path):
                        # 调度器中已经存在该任务路径
                        if task_path in self.tasks.keys():
                            # 判断是否有更改
                            if os.path.getmtime(conf_path) > self.tasks[task_path]['last_check']:
                                logger.info("Change task:{} cron time: {}".format(task_path, self.tasks[task_path]['cron']))
                                # 已经存在的任务
            ...
            for key, value in self.tasks.items():
                if value['last_check'] != now_time:
                    logger.error("Lost task conf:{}".format(key))
                    self.remove_task(key)
            time.sleep(10)

    def start(self):
        self.scan_running = True
        # self.scan_task_config()
