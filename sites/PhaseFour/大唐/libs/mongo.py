...
class MongDb(object):
    ...
    def traverse_batch(self, table, where=None, batch_size=500):
        cursor = None
        try:
            where = {} if where is None else where
            cursor = self.db[table].find(where, no_cursor_timeout=True).batch_size(batch_size)
            for item in cursor:
                yield item
        except WriteError as e:
            self.log.error('mongo写入异常:')
    ...
    # 重写save
    def update_save(self, table, data, is_upsert=True):
        ...
        if _id is None:
            return
        self.update(table, {'_id': _id}, {"$set": copy_data}, s_upsert=is_upsert)
