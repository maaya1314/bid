...
def process_url(base_url, deal_url):  # 处理url
    if deal_url is None or not isinstance(deal_url, str):
        return None
    if len(deal_url) == 0:
        return None
...
def decode_url(string):  # 处理URL编码
    if not isinstance(string, str) or string.find("%") == -1:
        return string
...
def change_sql_array(array):
    ...
    for item in array:
        ...
        elif isinstance(item, int) or isinstance(item, int) or isinstance(item, float):
            string += str(item) + ","
        elif item is None:
            string += "NULL,"
    if string != "[":
...
# def sort_merge_dedup(l, sort_func = cmp, eq_func = lambda a,b : a == b):
#     if len(l) <= 1:
#         return l
#     else:
...
def date_to_timestamp(date_str, need_double=False):
    time_stamp = time.mktime(datetime.datetime.strptime(date_str, "%Y-%m-%d %H:%M:%S").timetuple())
    time_stamp = time_stamp if need_double else int(time_stamp)
    return time_stamp
...
if __name__ == '__main__':
    ...
    date_time_before = "2018-11-09 14:00:00"

    print(date_to_timestamp(date_now) - date_to_timestamp(date_time_before))
