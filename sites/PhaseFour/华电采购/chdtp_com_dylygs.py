...
class BidZGDZ(TaskBase):
    ...
    def run(self):
        TIMEOUT = 60
        url = "https://www.chdtp.com/webs/detailDygs.action?chkedId=3CD074C5A9D4ACF9AA557142462057C63D4A4FA722730C93"
        self.headers = {
            ...
            if '基本信息' in temp:
                ...
                for j in temp_list:
                    if "采购单位" in j.xpath('string(./td[@class="td_1"])'):
                        data['招标人'] = j.xpath('string(./td[@class="td_2"])').strip()
                    elif "项目名称" in j.xpath('string(./td[@class="td_1"])'):
                        data['项目名称'] = j.xpath('string(./td[@class="td_2"])').strip()
                    elif "供应商" in j.xpath('string(./td[@class="td_1"])'):
                        data['中标单位'] = j.xpath('string(./td[@class="td_2"])').strip()
            elif '单一来源理由' in temp:
                data['招标条件'] = i.xpath('string(.//div[@class="tabmenu"]/following-sibling::div)').strip()
            elif '采购内容' in temp:
                data['招标范围'] = i.xpath('string(.//div[@class="tabmenu"]/following-sibling::div)').strip()
        # print(data)
        self.upload(data)
