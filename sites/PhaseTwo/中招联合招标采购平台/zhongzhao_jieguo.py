...
class BidZZLH(Bid3):
    ...
    def __init__(self, debug=True):
        ...
        self.exit_counts = 0
        self.file_name = '中招联合招标采购平台-结果公示'
        self.parse_dict = parse_dict.get(self.file_name)

    def run(self, keyword):
        TIMEOUT = 60
        self.keyword = keyword
        url_base = 'http://www.365trade.com.cn/jggs/index_{}.jhtml'
        self.headers = {
    ...
    def list_parse(self, content, url):
        ...
        html = etree.HTML(content)
        items = html.xpath("//ul[@class='searchList']/li")
        # url_list = re.findall("toGetContent\('(.*?)'\)", content)
        for item in items:
            if self.exit_flag:
                return
            # item_id = item.get("id")
            detail_href = item.xpath("string(./a[1]/@href)")
            # detail_url = re.findall("urlOpen\('(.*?)'\)", href_str)[0]
            detail_url = urljoin(url, detail_href)
            # detail_url = 'http://www.365trade.com.cn/jhwzb/334172.jhtml'
            headers = {
            ...
            if not detail_content:
                ...
                continue
            data = {}
            # data['project_title'] = item.xpath("string(./a[1]/p/span)").replace("\n", "").replace("\r", "").replace(" ", "")
            data['publish_time'] = item.xpath("string(./a[1]/i)").replace("发布日期：", "")
