...
class BidSZSZF(Bid3):
    ...
    def __init__(self, debug=True):
        ...
        self.exit_counts = 0
        self.file_name = '深圳市政府采网-招标公告'
        self.parse_dict = parse_dict.get(self.file_name)

    def run(self, keyword):
        TIMEOUT = 60
        self.keyword = keyword
        url_base = 'http://zfcg.szggzy.com:8081/gsgg/002001/002001002/002001002001/{}.html'
        self.headers = {
        ...
        # pages = html.xpath("string(//div[@class='pag-txt']/em[2])")
        counts = re.findall("total: (\d+),", content)[0]
        counts = int(counts)
        pages = counts // 10 if counts % 10 == 0 else int(counts) // 10 + 1
    ...
    def list_parse(self, content, url):
        ...
        html = etree.HTML(content)
        items = html.xpath("//ul[@id='infoContent']/li")
        # url_list = re.findall("toGetContent\('(.*?)'\)", content)
        for item in items:
            if self.exit_flag:
                return
            # item_id = item.get("id")
            detail_href = item.xpath("string(./a/@href)")
            # detail_url = re.findall("urlOpen\('(.*?)'\)", href_str)[0]
            detail_url = urljoin(url, detail_href)
            # detail_url = 'http://zfcg.szggzy.com:8081/gsgg/002001/002001002/002001002001/20220608/3fde8ef7-47f8-49b8-99c1-1d9847fd57e4.html'
            headers = {
            ...
            if not detail_content:
                ...
                continue
            data = {}
            data['project_title'] = item.xpath("string(./a)").split("]")[-1].replace("\n", "").replace("\r", "").replace("\t", "")
            data['publish_time'] = item.xpath("string(./span[@class='news-time'])")
