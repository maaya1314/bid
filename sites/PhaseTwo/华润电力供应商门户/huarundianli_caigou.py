...
class Bid2HRDL(Bid2):
    ...
    def __init__(self, debug=True):
        ...
        self.exit_counts = 0
        self.file_name = '华润电力供应商门户-寻源信息查询（物资、工程服务、副产品采购、燃料）'
        self.parse_dict = parse_dict.get(self.file_name)

    def run(self, keyword):
        TIMEOUT = 60
        self.keyword = keyword
        url = 'https://b2b.crpower.com.cn/ispweb/pcux5PonHeaders/searchWinningList.do'
        ...
        range_list = ['公开', '邀请']
        status_list = ['ACTIVE', 'CLOSED', 'AWARD_OPENED,AWARD_OPENED_BUS,AWARD_OPENED_TECH', 'AWARD_INPROCESS,AWARD_APPROVED,AWARD_REJECTED', 'AWARD_COMPLETED']
        # status_list = ['待报价', '已截止', '已开标', '决标处理中', '已创建合同']
        for range_word in range_list:
            for status_word in status_list:
                page = 1
                payload = {
                    'data': '{{"sourceMethods":"{}","sourceStatus":"{}","queryAll":"{}","sourceTypeTag":"","startDate":"","endDate":""}}'
                        .format(range_word, status_word, self.keyword),
                    'take': '60',
                    'skip': '0',
                    'page': page,
                    'pageSize': '60',
    ...
    def list_parse(self, content, url):
        ...
        for item in items:
            ...
            detail_url = 'https://b2b.crpower.com.cn/ispweb/pcux5PonHeaders/toFindSourceDetails.do?sourceId={}'.format(item_id)
            # headers = {
            ...
            data['bid_evaluation_rule'] = "{};{}".format(detail_item.get('qualificationRequirements'), detail_item.get('performanceRequirements'))
            data['bid_end_time'] = detail_item.get('quotationDate')

            # data[''] = detail_item.get("")
            ...
            ori_url = 'https://b2b.crpower.com.cn/ispweb/pcux5PonHeaders/souringdetail.do?sourceId={}'.format(item_id)
            self.detail_parse(detail_content, ori_url, data)
