...
class BidZGSY(Bid3):
    ...
    def __init__(self, debug=True):
        ...
        self.exit_counts = 0
        self.file_name = '国义-招标公告及资格预审公告'
        self.parse_dict = parse_dict.get(self.file_name)

    def run(self, keyword):
        TIMEOUT = 60
        self.keyword = keyword
        url_base = 'https://www.gmgit.com/Handlers/Default/Notice_BidInfo_List.ashx?categoryA=%E5%9B%BD%E9%99%85%E5%9B%BD%E5%86%85&categoryB=%E7%AE%A1%E8%BE%96%E9%83%A8%E9%97%A8&categoryC=%E8%A1%8C%E4%B8%9A%E5%88%86%E7%B1%BB&wd={}&thisPage={}&rowsEachPage=15&_=1652849742730'
    ...
    def list_parse(self, content, url):
        ...
        for item in items:
            ...
            detail_url = 'https://www.gmgit.com/Handlers/Bid/Notice_BidInfo_Detail.ashx?SNID={}'.format(item_id)
            ori_detail_url = 'https://www.gmgit.com/Bid/BidInfoDetail?SNID={}'.format(item_id)
            headers = {
            ...
            if isinstance(detail_item, str):
                self.log.error(detail_item)
                continue
            detail_content = detail_item.get("HTMContent")
            if not detail_content:
                detail_content = detail_item.get("TxtContent")
            attachment_url = detail_item.get("ET_AffixLink")
            if 'href' in attachment_url:
                attachment_url = "" if not re.findall("href='(.*?)'", attachment_url) else re.findall("href='(.*?)'", attachment_url)[0]
