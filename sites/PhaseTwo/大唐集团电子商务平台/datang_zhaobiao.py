...
class Bid2DT(Bid2):
    ...
    def __init__(self, debug=True):
        ...
        self.exit_counts = 0
        self.file_name = '大唐集团电子商务平台-招标公告'
        self.parse_dict = parse_dict.get(self.file_name)

    def run(self, keyword):
        TIMEOUT = 60
        self.keyword = keyword
        home_url = 'https://www.cdt-ec.com/notice/moreController/toMore?globleType=0'
        home_headers = {
    ...
    def slide(self, content):
        if "访问验证" in content:
            # 事件参数对象
            actionChains = ActionChains(self.driver)
            # 捕捉滑块元素
            slide_btn = self.driver.find_element_by_id('nc_1_n1z')
            # 观察网站滑块移动的长度和位置
            actionChains.drag_and_drop_by_offset(slide_btn, 100, 0).perform()
            time.sleep(1)
