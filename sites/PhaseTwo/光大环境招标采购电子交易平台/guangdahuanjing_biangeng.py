...
class Bid2ZGSY(Bid2):
    ...
    def __init__(self, debug=True):
        ...
        self.exit_counts = 0
        self.file_name = '光大环境招标采购电子交易平台-变更公告'
    ...
    def run(self, keyword):
        ...
        tab = '变更公告'
        category = '3'
        url_base = 'https://zcpt.cebenvironment.com.cn/cms/category/iframe.html?.html?searchDate=1997-04-20&dates=300&categoryId={}&tabName={}&precise=&status=&tenderno=&word={}&tenderMethod=&page={}'
        self.headers = {
    ...
    def list_parse(self, content, url):
        ...
        html = etree.HTML(content)
        items = html.xpath("//ul[@class='newslist']/li")
        # url_list = re.findall("toGetContent\('(.*?)'\)", content)
        for item in items:
            if self.exit_flag:
                return
            # item_id = item.get("id")
            href_str = item.xpath("string(./a/@href)")
            # detail_url = re.findall("urlOpen\('(.*?)'\)", href_str)[0]
            detail_url = href_str
            # detail_url = 'https://zcpt.cebenvironment.com.cn/ebi_changeBulletin/2020-04-28/2328.html'
            headers = {
            ...
            if not detail_content:
                ...
                continue
            data = {}
            # data['project_title'] = item.xpath('string(./a/h1)').replace("\n", "").replace("\r", "").replace(" ", "")
            data['publish_time'] = item.xpath("string(./a/div[@class='newsDate'])").replace("\n", "").replace("\r", "").replace("\t", "")
    ...
    def fix_data(self, data, detail_content):
        attachment_id = re.findall("downCgUploadData\('(.*?)'", detail_content)
        if attachment_id:
            attachment_url = 'https://zcpt.cebenvironment.com.cn/dzzb/cgUploadController.do?downLoadFile&objId=' + attachment_id
