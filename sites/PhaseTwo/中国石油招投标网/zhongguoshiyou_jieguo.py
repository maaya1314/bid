...
class BidZGSY(Bid3):
    ...
    def __init__(self, debug=True):
        ...
        self.exit_counts = 0
        self.file_name = '中国石油招投标网-公开招标中标结果公示'
    ...
    def list_parse(self, content, url):
        ...
        for item in items:
            ...
            for detail_item in detail_items:
                for k, v in detail_item.items():
                    if not v:
                        detail_item[k] = ""
                if bid_winner:
                    bid_winner += "," + detail_item.get("tendername")
                else:
                    bid_winner += detail_item.get("tendername")
                if win_bid_price:
                    win_bid_price += "," + detail_item.get("quote") + '元'
                else:
                    win_bid_price += detail_item.get("quote") + '元'
                ...
                data['win_bid_price'] = win_bid_price
            ...
            for attachment in attachment_items:
                ...
                if attachment_url:
                    attachment_url += "," + "https://www.cnpcbidding.com/pmsbid/download?id={}&skip=true".format(
                        attachment_id)
                else:
                    attachment_url += "https://www.cnpcbidding.com/pmsbid/download?id={}&skip=true".format(
                        attachment_id)
            data['attachment_url'] = attachment_url
            done_fields = ['project_number', 'tender_unit', 'bid_winner', 'win_bid_price', 'agency', 'phone', 'project_leader', 'bid_finish_time']
