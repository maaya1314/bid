...
class BidCY(Bid3):
    ...
    def __init__(self, debug=True):
        ...
        self.exit_counts = 0
        self.file_name = '中国华电集团电子商务平台-招标公告'
    ...
    def list_parse(self, content, url):
        ...
        for item in items:
            ...
            href = re.findall("toGetContent\('(.*?)'\)", href_str)[0]
            detail_url = urljoin(url, '/staticPage/' + href)
            # detail_url = 'http://www.zcbidding.com/portal/ggXq?id={}'.format(object_id)
            # detail_url = 'https://www.chdtp.com/staticPage/zhaobiaogg/2014/07/08/zhaobiaogg_350831_5670.html'
            headers = {
            ...
            if not detail_content:
                ...
                continue
            data = {}
            data['project_title'] = item.xpath("string(./td[2]/a/@title)")
            data['industry_type'] = item.xpath("string(./td[3])")
            data['publish_time'] = item.xpath("string(./td[4])").replace("[", '').replace("]", '')
