...
class BidZGSY(Bid3):
    ...
    def __init__(self, debug=True):
        ...
        self.exit_counts = 0
        self.file_name = '深圳市国际招标有限公司-变更公告'
        self.parse_dict = parse_dict.get(self.file_name)

    def run(self, keyword):
        TIMEOUT = 60
        self.keyword = keyword
        url_base = 'http://new.sztc.com/changeBulletin/index_{}.htm'
        self.headers = {
    ...
    def list_parse(self, content, url):
        ...
        for item in items:
            ...
            if not detail_content:
                ...
                continue
            data = {}
            data['project_title'] = item.xpath("string(./a/span[1])").replace("\n", "").replace("\r", "").replace(" ", "")
            data['publish_time'] = item.xpath("string(./a/span[2])").replace("\n", "").replace("\r", "").replace("\t", "")
